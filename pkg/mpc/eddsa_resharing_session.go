...
type eddsaReshareSession struct {
	...
	frostConfig    *frost.TaprootConfig
	newThreshold   int
	newNodeIDs     []string
...
func newEdDSAReshareSession(
	...
	if _, err := kvstore.Get(frostKey); err == nil {
	...
	// Load existing FROST config for old peers
	if !isNewPeer {
		config, err := s.loadFROSTConfig(walletID)
		if err != nil {
			return nil, fmt.Errorf("failed to load existing FROST config: %w", err)
		}
		s.frostConfig = config
	}
...
func (s *eddsaReshareSession) Init() {
	s.logger.Info().
		...
		Msg("[FROST-RESHARE] Initializing FROST reshare session")
	...
	if s.isNewPeer {
		...
		s.logger.Info().Msg("[FROST-RESHARE] Creating keygen for new peer")
		startFunc = frost.KeygenTaproot(s.selfPartyID, newIDs, s.newThreshold)
	} else {
		// Old peers run the refresh protocol
		// Note: FROST refresh doesn't support changing participants, only refreshing shares
		s.logger.Info().Msg("[FROST-RESHARE] Creating refresh for old peer")
		startFunc = frost.RefreshTaproot(s.frostConfig, newIDs)
	}

	// Create handler with proper context, logging, and session ID
	sessionIDBytes := []byte(fmt.Sprintf("frost-reshare-%s", s.walletID))
	...
	if err != nil {
		s.logger.Error().Err(err).Msg("[FROST-RESHARE] Failed to create handler")
		s.errCh <- err
		return
	}
	...
	s.logger.Info().
		...
		Strs("newNodeIDs", s.newNodeIDs).
		Int("newThreshold", s.newThreshold).
		Msg("[FROST-RESHARE] Session initialized successfully")
}
...
func (s *eddsaReshareSession) Reshare(done func()) {
	...
	s.logger.Info().
		Str("sessionID", s.sessionID).
		Bool("isNewPeer", s.isNewPeer).
		Msg("[FROST-RESHARE] Starting reshare protocol")
	...
	s.logger.Info().Msg("[FROST-RESHARE] Protocol finished, publishing result")
	s.publishResult()
}

// handleProtocolMessages handles messages from the protocol handler
func (s *eddsaReshareSession) handleProtocolMessages() {
	for {
		select {
		case protoMsg, ok := <-s.handler.Listen():
			if !ok {
				// Protocol finished
				s.logger.Info().Msg("[FROST-RESHARE] handler.Listen() returned !ok - protocol finished")
				s.resultMutex.Lock()
				s.done = true
				result, err := s.handler.Result()
				if err != nil {
					s.logger.Error().Err(err).Msg("[FROST-RESHARE] handler.Result() returned error")
					s.resultErr = err
					s.errCh <- err
				} else if cfg, ok := result.(*frost.TaprootConfig); ok {
					s.frostConfig = cfg
					s.logger.Info().Msg("[FROST-RESHARE] Got FROST Taproot config result")
				} else {
					s.logger.Error().Msgf("[FROST-RESHARE] Unexpected result type: %T", result)
			...
			if err != nil {
				s.logger.Error().Err(err).Msg("[FROST-RESHARE] Failed to marshal protocol message")
				continue
			}
			...
			s.logger.Debug().
				...
				Msg("[FROST-RESHARE] Protocol emitted message")
		...
		case protoMsg := <-s.messagesCh:
			...
			s.logger.Debug().
				...
				Bool("broadcast", protoMsg.Broadcast).
				Int("round", int(protoMsg.RoundNumber)).
				Msg("[FROST-RESHARE] Received protocol message, checking CanAccept")
			...
			if !s.handler.CanAccept(protoMsg) {
				s.logger.Warn().
					...
					Int("round", int(protoMsg.RoundNumber)).
					Str("selfPartyID", string(s.selfPartyID)).
					Msg("[FROST-RESHARE] Handler cannot accept message")
				continue
			}

			s.logger.Debug().
				Str("from", string(protoMsg.From)).
				Msg("[FROST-RESHARE] Handler accepted message")
...
func (s *eddsaReshareSession) ListenToIncomingMessageAsync() {
	...
	broadcastSub, err := s.pubSub.Subscribe(broadcastTopic, func(m *nats.Msg) {
		s.logger.Debug().
			...
			Int("size", len(m.Data)).
			Msg("[FROST-RESHARE] Received broadcast message")
		s.ProcessInboundMessage(m.Data)
	})

	if err != nil {
		s.logger.Error().Err(err).Msgf("[FROST-RESHARE] Failed to subscribe to broadcast topic %s", broadcastTopic)
		s.errCh <- err
		return
	}
	...
	directSub, err := s.pubSub.Subscribe(directTopic, func(m *nats.Msg) {
		s.logger.Debug().
			...
			Int("size", len(m.Data)).
			Msg("[FROST-RESHARE] Received direct message")
		s.ProcessInboundMessage(m.Data)
	})

	if err != nil {
		s.logger.Error().Err(err).Msgf("[FROST-RESHARE] Failed to subscribe to direct topic %s", directTopic)
		s.errCh <- err
		return
	}
	...
	s.logger.Info().
		...
		Msg("[FROST-RESHARE] Listening to incoming messages")
}
...
func (s *eddsaReshareSession) ProcessInboundMessage(msgBytes []byte) {
	...
	if err := json.Unmarshal(msgBytes, inboundMessage); err != nil {
		s.logger.Error().Err(err).Msg("[FROST-RESHARE] ProcessInboundMessage unmarshal error")
		return
	}
	...
	if err := protoMsg.UnmarshalBinary(inboundMessage.Body); err != nil {
		s.logger.Error().Err(err).Msg("[FROST-RESHARE] Failed to unmarshal protocol message")
		return
	}
	...
	s.logger.Debug().
		...
		Msg("[FROST-RESHARE] Received protocol message")
...
func (s *eddsaReshareSession) ProcessOutboundMessage() {
	s.logger.Info().Str("sessionID", s.sessionID).Msg("[FROST-RESHARE] ProcessOutboundMessage started")
	...
	for {
		...
		case err := <-s.errCh:
			s.logger.Error().Err(err).Msg("[FROST-RESHARE] Received error during ProcessOutboundMessage")

		case <-s.finishCh:
			s.logger.Info().Msg("[FROST-RESHARE] Received finish message during ProcessOutboundMessage")
			s.publishResult()
			return
		}
	}
...
func (s *eddsaReshareSession) publishResult() {
	s.resultMutex.Lock()
	defer s.resultMutex.Unlock()

	if s.resultErr != nil {
		s.logger.Error().Err(s.resultErr).Msg("[FROST-RESHARE] Reshare failed with error")
		// Always send to externalFinishChan so WaitForFinish() doesn't block forever
		s.externalFinishChan <- ""
		return
	}

	if s.frostConfig == nil {
		s.logger.Error().Msg("[FROST-RESHARE] No config available after reshare completion")
		s.externalFinishChan <- ""
		return
	}

	// Save the new key share with frost prefix
	shareBytes, err := json.Marshal(s.frostConfig)
	if err != nil {
		s.logger.Error().Err(err).Msg("[FROST-RESHARE] Failed to marshal key share")
		s.externalFinishChan <- ""
		return
	}
	...
	if err := s.kvstore.Put(frostKey, shareBytes); err != nil {
		s.logger.Error().Err(err).Msgf("[FROST-RESHARE] Failed to save key share for wallet %s", s.walletID)
		s.externalFinishChan <- ""
		return
	}
	...
	if err := s.keyinfoStore.Save(s.walletID, keyInfo); err != nil {
		s.logger.Error().Err(err).Msg("[FROST-RESHARE] Failed to save key info")
		s.externalFinishChan <- ""
		return
	}
	...
	if s.frostConfig != nil && len(s.frostConfig.PublicKey) > 0 {
		pubKeyHex = fmt.Sprintf("%x", s.frostConfig.PublicKey)
		s.pubKeyResult = s.frostConfig.PublicKey
	}
	...
	s.logger.Info().
		...
		Int("newThreshold", s.newThreshold).
		Strs("newNodeIDs", s.newNodeIDs).
		Msg("[FROST-RESHARE] Reshare completed successfully")

	// Notify via external finish channel
	s.externalFinishChan <- pubKeyHex
}

// loadFROSTConfig loads the existing FROST Taproot configuration
func (s *eddsaReshareSession) loadFROSTConfig(walletID string) (*frost.TaprootConfig, error) {
	// Load the key share data with frost prefix
	frostKey := fmt.Sprintf("frost:%s", walletID)
	keyShareData, err := s.kvstore.Get(frostKey)
	if err != nil {
		return nil, fmt.Errorf("failed to get FROST key share: %w", err)
	}

	// Deserialize to FROST Taproot config
	var config frost.TaprootConfig
	if err := json.Unmarshal(keyShareData, &config); err != nil {
		return nil, fmt.Errorf("failed to unmarshal as FROST Taproot config: %w", err)
	}
