...
// landingTemplate is the marketing surface served on GET /. All visible
// brand-specific text (title, wordmark, sign-in label, footer links)
// flows through {{.Brand}}; the empty Brand renders a generic page.
const landingTemplate = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{{.Brand.TitlePrefix}}MPC — Enterprise Digital Asset Infrastructure</title>
...
<div class="container">
  ...
  <div class="nav-links">
    ...
    {{if .Brand.DocsURL}}<a href="{{.Brand.DocsURL}}">Docs</a>{{end}}
    {{if .Brand.IAMURL}}<a href="#" onclick="startLogin()" class="btn btn-primary btn-sm">Sign In</a>{{end}}
  </div>
</div>
</header>
...
<div class="container">
  ...
  <div class="ctas">
    {{if .Brand.IAMURL}}<a href="#" onclick="startLogin()" class="btn btn-primary">{{.Brand.SignInLabel}}</a>{{end}}
    <a href="/health" class="btn btn-outline">API Status</a>
    {{if .Brand.GitHubURL}}<a href="{{.Brand.GitHubURL}}" class="btn btn-outline">GitHub</a>{{end}}
  </div>
</div>
</div>
...
<div class="container">
  ...
  <div class="protocols">
    ...
    <div class="protocol">
      ...
      <p>Quantum-Safe And Reliable. Combines BLS12-381 pairing-based signatures with a lattice scheme for immediate and future-proof security.</p>
    </div>
    <div class="protocol">
      <h3>BLS</h3>
...
<div class="container">
  ...
  <div class="features">
    ...
    <div class="feature">
      ...
      <p>Lattice-based threshold signatures and the Quasar hybrid scheme provide protection against future quantum computer attacks at configurable security levels.</p>
    </div>
    <div class="feature">
    ...
    <div class="feature">
      <h3>Unified IAM</h3>
      <p>All operations authenticated via{{if .Brand.HasBrand}} {{.Brand.Name}} ID{{else}} an OIDC identity provider{{end}}. Bearer token validation, role-based access control, and audit logging for compliance.</p>
    </div>
  </div>
</div>
</section>
...
<div class="container">
  ...
  <div class="features">
    ...
    <div class="feature">
      ...
      <p>LSSS share refresh and lattice-scheme proactive rotation ensure long-term security even if shares are periodically exposed. Generation-based key lifecycle.</p>
    </div>
    <div class="feature">
...
<div class="container">
  ...
  <p>Get started with {{.Brand.TitlePrefix}}MPC in minutes. Self-hosted or managed.</p>
  <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
    {{if .Brand.IAMURL}}<a href="#" onclick="startLogin()" class="btn btn-primary">{{.Brand.SignInLabel}}</a>{{end}}
    {{if .Brand.DocsURL}}<a href="{{.Brand.DocsURL}}" class="btn btn-outline">Read the Docs</a>{{end}}
    {{if .Brand.GitHubURL}}<a href="{{.Brand.GitHubURL}}" class="btn btn-outline">GitHub</a>{{end}}
  </div>
</div>
</div>
...
<div class="container">
  ...
  <div style="display:flex;gap:20px">
    {{if .Brand.SiteURL}}<a href="{{.Brand.SiteURL}}">{{.Brand.SiteURL}}</a>{{end}}
    {{if .Brand.DocsURL}}<a href="{{.Brand.DocsURL}}">Docs</a>{{end}}
    {{if .Brand.GitHubURL}}<a href="{{.Brand.GitHubURL}}">GitHub</a>{{end}}
  </div>
</div>
</footer>
<script>
...
async function startLogin() {
  if (!IAM_URL || !CLIENT_ID) { alert('Sign-in is not configured for this deployment.'); return; }
...
// authCallbackTemplate handles the OAuth code-exchange flow.
const authCallbackTemplate = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{{.Brand.TitlePrefix}}MPC — Authenticating</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
<div class="card">
  ...
  <p id="status">Exchanging authorization code{{if .Brand.HasBrand}} with {{.Brand.Name}} ID{{end}}.</p>
...
async function handleCallback() {
  ...
  // Direct token (from worker bridge flow)
  ...
  if (!IAM_URL || !CLIENT_ID) { showError('Sign-in is not configured for this deployment.'); return; }

  const savedState = sessionStorage.getItem('oauth_state');
  if (savedState && state !== savedState) { showError('State mismatch — possible CSRF attack'); return; }
...
// dashboardTemplate is the post-OAuth dashboard.
const dashboardTemplate = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{{.Brand.TitlePrefix}}MPC — Dashboard</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{background:#000;color:#d4d4d4;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;min-height:100vh;display:flex;flex-direction:column;-webkit-font-smoothing:antialiased}
a{color:#fff;text-decoration:none}
...
<main>
<div class="container">
  <h1>{{.Brand.TitlePrefix}}MPC Dashboard</h1>
...
async function apiCall(method, path, body) {
  ...
  res.textContent = method + ' ' + path + ' ...';
  try {
    const opts = { method, headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' } };
    if (body) opts.body = JSON.stringify(body);
    const resp = await fetch(API_BASE + path, opts);
    const txt = await resp.text();
    try { res.textContent = JSON.stringify(JSON.parse(txt), null, 2); } catch (_) { res.textContent = txt; }
  } catch (e) {
