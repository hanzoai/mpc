...
func toCMPConfig(cfg protocol.KeyGenConfig) (*config.Config, error) {
	...
	// For other types, try to deserialize from the stored data
	// This handles configs loaded from kvstore
	serialized, err := cfg.Serialize()
	if err != nil {
		return nil, fmt.Errorf("failed to serialize config for conversion: %w", err)
	}

	// Deserialize back to *config.Config
	cmpConfig := &config.Config{}
	if err := json.Unmarshal(serialized, cmpConfig); err != nil {
