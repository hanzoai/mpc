...
import (
	...
	authpkg "github.com/hanzoai/mpc/pkg/auth"
...
// runConsensus is the canonical luxfi/mpc consensus-mode entry point.
// Invoked from main.go when the operator passes the `consensus`
// subcommand or sets MPCD_MODE=consensus. The thin embed path
// (pkg/mpc.Embed) is the default; consensus mode is opt-in for the
// live cluster.
func runConsensus() {
...
func runNodeConsensus(ctx context.Context, c *cli.Command) error {
	...
	if apiAddr != "" {
		...
		mux.HandleFunc("/keygen", internalAuth(internalRateLimit(func(w http.ResponseWriter, r *http.Request) {
			...
			// Identity is the X-Org-Id header populated upstream by
			// hanzoai/gateway after IAM JWT validation. Reading orgID
			// from a body field is a forgery vector and is removed
			// (see MIGRATION.md). Solo-mode dev path: header empty,
			// audit log records "" — caller deals with the consequence.
			orgID := r.Header.Get(authpkg.HeaderOrgID)
			if orgID == "" {
				w.WriteHeader(http.StatusBadRequest)
				json.NewEncoder(w).Encode(map[string]string{"error": "X-Org-Id header is required"})
				return
			}
			...
			logger.Info("Audit: keygen triggered", "nodeID", nodeID, "orgID", orgID, "walletID", walletID, "remote", r.RemoteAddr)
